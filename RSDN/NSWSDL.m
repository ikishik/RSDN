...
-(SoapRequest*) buildSoapRequest: (NSString*) methodName withNS:(NSString*)nsUri withAction:(NSString*)action withHeader:(NSString*)header error:(NSError**) pError
{
	...
	NSString* nsName = [self soapEnvelopeNS];
	...
	//soapRequest = [soapRequest stringByReplacingOccurrencesOfString:@"SOAPENVELOPE" withString:nsName];
	//soapRequest = [soapRequest stringByReplacingOccurrencesOfString:@"SOAPHEADER" withString:headerXml];
    
    NSString* soapRequest = [NSString stringWithFormat:@"<?xml version='1.0' encoding='utf-8'?><s:Envelope xmlns:s='http://schemas.xmlsoap.org/soap/envelope/'><s:Body><SOAPREQUEST xmlns='%@'xmlns:i='http://www.w3.org/2001/XMLSchema-instance'></SOAPREQUEST></s:Body></s:Envelope>", nsUri];
	soapRequest = [soapRequest stringByReplacingOccurrencesOfString:@"SOAPREQUEST" withString:methodName];
	//soapRequest = [soapRequest stringByReplacingOccurrencesOfString:@"SOAPENVELOPE" withString:nsName];
	soapRequest = [soapRequest stringByReplacingOccurrencesOfString:@"HSOAPREQUEST" withString:headerXml];
	NSXMLDocument* doc = [NSWSDL documentWithText:soapRequest orData:nil options:NSXMLDocumentValidate error:pError];
	[req setDocument:doc];
	NSXMLElement* root = [doc rootElement];
	if(header!=nil){
		[req setHeader: [NSWSDL getElementNS:root forNamespace:nsName forName:@"Header"] ];
	}
	NSXMLElement* body = [NSWSDL getElementNS:root forNamespace:nsName forName:@"Body"];
...
+(NSDate*)  getDate: (NSXMLElement*) root: (NSString*) name: (BOOL) bAttribute{
	...
	if(val==nil)
		return nil;
	NSDate* dt = [NSWSDL parseDate:val withFormat:@"yyyy'-'MM'-'dd'T'HH':'mm':'ss'Z'"];
	if(dt!=nil)
		return dt;
